...
- Static Quartz/Pages site has no runtime server: org-level GitHub Discussions are readable only via GraphQL+auth token (no anon REST), so post bodies can't be fetched from a static page. 'Render live from GitHub' needs a credentialed middle layer (serverless proxy, like giscus's hosted backend) — else build-time sync to markdown before quartz build.
