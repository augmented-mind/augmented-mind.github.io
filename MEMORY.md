...
- Keep Claude default pinned to claude-opus-4-8; do NOT downgrade to ACP 'opus' alias (PR #72 closed for this). Pins need acpx ≥0.10.0 (0.6.1 only advertised default/opus/haiku). Resumed sessions advertise only default/opus/haiku, so fix the resume path to surface the pin to ACP — bump acpx, don't add adapter glue or alias fallbacks.
