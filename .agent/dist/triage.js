...
exports.extractRequestedRoute = extractRequestedRoute;
exports.extractRequestedRouteDecision = extractRequestedRouteDecision;
exports.buildRequestedRouteDecision = buildRequestedRouteDecision;
exports.resolveRequestedLabel = resolveRequestedLabel;
exports.normalizeDispatch = normalizeDispatch;
exports.applyDispatchPolicy = applyDispatchPolicy;
const mentions_js_1 = require("./mentions.js");
const response_js_1 = require("./response.js");
const access_policy_js_1 = require("./access-policy.js");
exports.ROUTES = new Set([
    "answer",
    "implement",
    "fix-pr",
    "review",
...
const EXPLICIT_ROUTE_COMMANDS = ["answer", "implement", "fix-pr", "review", "create-action"];
 ...
 */
function extractRequestedRoute(body, mention) {
 ...
 */
function extractRequestedRouteDecision(body, mention) {
    const sanitized = (0, mentions_js_1.stripNonLiveMentions)(String(body || ""));
    const trimmedMention = String(mention || "").trim();
    if (!sanitized.trim() || !trimmedMention) {
        return { route: "", skill: "" };
    }
    const routePattern = EXPLICIT_ROUTE_COMMANDS.map((route) => (0, mentions_js_1.escapeRegex)(route)).join("|");
    const explicitRegex = new RegExp(`(?:^|[\\s(])${(0, mentions_js_1.escapeRegex)(trimmedMention)}\\s+/(${routePattern})(?=$|[\\s.,;:!?)\\]}])`, "im");
    const explicitMatch = sanitized.match(explicitRegex);
    if (explicitMatch) {
        return { route: explicitMatch[1].toLowerCase(), skill: "" };
    }
    const skillRegex = new RegExp(String.raw `(?:^|[\s(])${(0, mentions_js_1.escapeRegex)(trimmedMention)}\s+/skill\s+([A-Za-z0-9][A-Za-z0-9._-]*)(?=$|[\s.,;:!?)\]}])`, "im");
 ...
 */
function buildRequestedRouteDecision(route, requestText) {
    const normalizedRoute = String(route || "").trim().toLowerCase();
    if (normalizedRoute !== "skill" &&
        !EXPLICIT_ROUTE_COMMANDS.includes(normalizedRoute)) {
 ...
 */
function resolveRequestedLabel(labelName) {
 ...
 */
function applyDispatchPolicy(decision, targetKind, authorAssociation, accessPolicy = { routeOverrides: {} }, isPublicRepo = false, isExplicit = false) {
    const normalized = { ...decision };
    if (String(authorAssociation || "").trim() &&
        !(0, access_policy_js_1.isAssociationAllowedForRoute)(accessPolicy, normalized.route, authorAssociation || "", isPublicRepo)) {
        const allowed = (0, access_policy_js_1.getAllowedAssociationsForRoute)(accessPolicy, normalized.route, isPublicRepo);
        return {
            ...normalized,
            route: "unsupported",
            needsApproval: false,
    ...
    if (normalized.route === "fix-pr") {
        if (targetKind !== "pull_request") {
            return {
                ...
                route: "unsupported",
                needsApproval: false,
                summary: "PR fix requests are only supported from pull requests right now.",
                issueTitle: "",
                issueBody: "",
            };
        }
    ...
    if (normalized.route === "review") {
        if (targetKind !== "pull_request") {
            return {
                ...
                route: "unsupported",
                needsApproval: false,
                summary: "Review requests are only supported from pull requests right now.",
                issueTitle: "",
                issueBody: "",
            };
        }
